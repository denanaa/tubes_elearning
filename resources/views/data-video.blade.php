    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        <tbody>
            ...
            <tr class="odd:bg-white even:bg-gray-50 border-b text-center">
                ...
                <td class="px-6 py-4">
                    <button type="button" onclick="showEditModal('{{ $video->id_video }}', '{{ $video->title_video }}', '{{ $video->link_video }}', '{{ asset('storage/' . $video->thumbnail_video) }}', '{{ $video->description_video }}', '{{ $video->id_module }}')" class="bg-green-500 text-white text-xs hover:bg-green-600 font-medium me-2 px-4 py-0.5 rounded">
                        Edit
                    </button>
...
<!-- Modal Edit -->
<div id="editModal-{{ $video->id_video }}" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center">
    <div class="bg-white p-6  w-[90%] max-w-lg border rounded-lg shadow-sm bg-card text-neutral-900">
        ...
        <form action="{{ route('update-video', $video->id_video) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="my-4 space-y-1">
                <label for="editModul-{{ $video->id_video }}" class="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">Modul</label>
                <select id="editModul-{{ $video->id_video }}" name="id_module" class="flex w-full h-10 px-3 py-2 text-sm bg-white border rounded-md peer border-neutral-300 ring-offset-background placeholder:text-neutral-400 focus:border-neutral-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-neutral-400 disabled:cursor-not-allowed disabled:opacity-50">
                    <option value="" disabled>Select a module</option>
                    @if(isset($modules) && $modules->count())
                        @foreach($modules as $module)
                            <option value="{{ $module->id_module }}" {{ $video->id_module == $module->id_module ? 'selected' : '' }}>
        ...
        onchange="previewImage(event)">
            ...
            <div class="my-4 space-y-1">
                ...
                <textarea type="text" id="editDescription" name="description_video" rows ="6" class="flex w-full px-3 py-2 text-sm bg-white border rounded-md peer border-neutral-300 ring-offset-background placeholder:text-neutral-400 focus:border-neutral-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-neutral-400 disabled:cursor-not-allowed disabled:opacity-50" placeholder="Description Video">{{ old('description_video', $video->description_video) }}</textarea>
            </div>
            <div class="flex justify-end">
                <button type="button" onclick="hideEditModal('{{ $video->id_video }}')" class="mt-3 inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto">Cancel</button>
...
<!-- Modal Hapus -->
<div id="deleteModal-{{ $video->id_video }}" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center">
    <div class="bg-white p-6 rounded-lg shadow-lg w-[90%] max-w-sm">
        ...
        <form action="{{ route('delete-video', ['id_video' => $video->id_video]) }}" method="POST" >
            ...
            <div class="flex justify-end ">
                <button type="button" onclick="hideDeleteModal('{{ $video->id_video }}')" class="mt-3 inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto">Cancel</button>
