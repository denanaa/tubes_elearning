<div class="w-full">
    <div class="flex items-center justify-between w-full relative">
        ...
        <div class="flex items-center px-4">
            ...
            <!-- Menu Navigasi -->
            <nav :class="{ 'hidden': !isHamburgerActive }"
                        ...
                        @else
                            <div x-data="{ open: false }" @click.outside="open=false" class="relative text-sm py-2 pr-4">
                                ...
                                <ul x-show="open"
                                    ...
                                    x-transition x-cloak href="{{ route('logout') }}">
                                    <!-- Tampilkan Dashboard hanya untuk Admin -->
                                    @if (Auth::user() && Auth::user()->role === 'admin')
                                        <a href="{{ route('dashboard') }}"
                                            class="relative flex cursor-default select-none hover:bg-neutral-100 items-center rounded px-2 py-1.5 text-sm outline-none transition-colors focus:bg-sky focus:text-accent-foreground">
                                            Dashboard
                                        </a>
                                    @endif
                                    <a href="{{ route('logout') }}"
