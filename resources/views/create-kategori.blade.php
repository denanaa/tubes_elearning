...
@section('content')
    <div class="container">
        <form action="{{ isset($category) ? route('update-kategori', $category->id_category) : route('store-kategori') }}"
            ...
            <div class="my-4 space-y-1">
                <label for="editDescription"
                    ...
                    Category</label>
                <textarea type="text" id="editDescription" name="description" rows ="6" {{ $category->description ?? '' }}
            ...
            <div class="flex justify-start">
                <button type="submit"
                    class="mt-3 inline-flex w-full justify-center rounded-md bg-blue-500  px-3 py-2 text-sm font-semibold text-white shadow-sm ring-gray-300 hover:bg-blue-600 sm:mt-0 sm:w-auto">Save</button>
                <a href="{{ route('data-kategori') }}">
                    <button type="button" onclick="hideEditModal()"
                        class="inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:ml-3 sm:w-auto">Cancel</button>
                </a>
            </div>
        </form>
    </div>
